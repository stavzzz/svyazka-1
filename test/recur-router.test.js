...
test('серия п.23 (баг 24.07 утро): две встречи одной фразой → предупреждение о второй, первая в работе', async () => {
  const deps = makeDeps({
    classifierMap: {
      'Тест 1': {
        intent: 'create', title: 'Тест 1', date: '2026-07-25', time_start: '17:00', duration_min: '60',
        city: 'Москва', more_titles: ['Тест 2'],
        recur: { freq: 'weekly', byday: ['SA'], interval: '1', count: '', until: '' },
      },
    },
  });
  const router = createRouter(deps);
  await router.refreshCache();
  await router.handleUpdate(msg('Поставь каждую СБ встречу Тест 1 в 17:00 МСК и каждое ВС Тест 2 в 18:00 МСК'));
  const note = deps.tg.sent.find((m) => m.html.includes('По одной за раз'));
  assert.ok(note, 'нет предупреждения о второй встрече');
  assert.ok(note.html.includes('<b>Тест 2</b>'));
  assert.ok(deps.tg.sent.at(-1).html.startsWith('<b>📅 Докуда повторять?</b>')); // первая пошла в работу
});
