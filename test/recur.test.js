...
test('parseRecurPhrase: сокращения ПН/ПТ/ВС (баг приёмки 23.07 ночь)', () => {
  assert.deepEqual(parseRecurPhrase('каждый ПН в 6:30 утра'),
    { freq: 'weekly', byday: ['MO'], interval: 1 });
  assert.deepEqual(parseRecurPhrase('каждый пн и пт'),
    { freq: 'weekly', byday: ['MO', 'FR'], interval: 1 });
  assert.deepEqual(parseRecurPhrase('по ВТ и ЧТ'),
    { freq: 'weekly', byday: ['TU', 'TH'], interval: 1 });
  // Без контекста повторения сокращение — не серия
  assert.equal(parseRecurPhrase('поставь встречу в ПТ в 15:00'), null);
});
