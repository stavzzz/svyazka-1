...
test('№10 «в 15:00» и «на 8 утра» вопросов не вызывают', async () => {
  const deps = makeDeps({ classifierMap: {
    ...
    'восемь утра': { intent: 'create', title: 'Б', date: '2026-07-23', time_start: '08:00' },
  } });
  const router = createRouter(deps);
  await router.refreshCache();
...
// ── Правки 23.07 (ночь-2): чистка названия из ответа + кнопки «Нашёл встречу» ──
test('ответ «назови встречу "Тест"» — в название попадает только Тест', async () => {
  const deps = makeDeps({ classifierMap: {
    'поставь встречу': { intent: 'create', title: '', date: '2026-07-23', time_start: '15:00' },
  } });
  const router = createRouter(deps);
  await router.refreshCache();
  await router.handleUpdate(msg('поставь встречу завтра на 15:00'));
  const ask = deps.tg.sent.at(-1);
  await router.handleUpdate(msg('назови встречу "Адский тест"', { reply_to_message: { message_id: ask.message_id, text: ask.html } }));
  const created = deps.gcal.calls.find((c) => c[0] === 'create');
  assert.equal(created[1].summary, 'Адский тест');
});

test('кнопки «Нашёл встречу»: Переименовать работает', async () => {
  const deps = makeDeps({
    classifierMap: { 'найди': { intent: 'find', titles: ['тест кнопок'] } },
    gcalOpts: { events: [rawEvent('f1', 'Тест кнопок', '2026-07-24T15:00:00', '2026-07-24T16:00:00')] },
  });
  const router = createRouter(deps);
  await router.refreshCache();
  await router.handleUpdate(msg('найди тест кнопок'));
  const card = deps.tg.sent.at(-1);
  const flat = card.opts.buttons.flat();
  assert.deepEqual(flat.map((b) => b.text), ['🔁 Перенести', '✏️ Переименовать', '👥 Пригласить', '🗑 Удалить']);
  await router.handleUpdate(cb(flat[1].callback_data));           // ✏️
  assert.match(deps.tg.sent.at(-1).html, /Новое название/);
  const ask = deps.tg.sent.at(-1);
  await router.handleUpdate(msg('Продажный тест', { reply_to_message: { message_id: ask.message_id, text: ask.html } }));
  const patch = deps.gcal.calls.find((c) => c[0] === 'patch');
  assert.equal(patch[2].summary, 'Продажный тест');
  assert.match(deps.tg.sent.at(-1).html, /обновлена/);
});

test('кнопки «Нашёл встречу»: Удалить → подтверждение → удалена; Пригласить добавляет почту', async () => {
  const deps = makeDeps({
    classifierMap: { 'найди': { intent: 'find', titles: ['тест кнопок'] } },
    gcalOpts: { events: [rawEvent('f2', 'Тест кнопок', '2026-07-24T15:00:00', '2026-07-24T16:00:00')] },
  });
  const router = createRouter(deps);
  await router.refreshCache();
  await router.handleUpdate(msg('найди тест кнопок'));
  const inv = deps.tg.sent.at(-1).opts.buttons.flat()[2].callback_data; // 👥
  await router.handleUpdate(cb(inv));
  const ask = deps.tg.sent.at(-1);
  assert.match(ask.html, /Кого пригласить/);
  await router.handleUpdate(msg('vasya@mail.ru', { reply_to_message: { message_id: ask.message_id, text: ask.html } }));
  const patch = deps.gcal.calls.find((c) => c[0] === 'patch');
  assert.deepEqual(patch[2].attendees, [{ email: 'vasya@mail.ru' }]);

  await router.handleUpdate(msg('найди тест кнопок'));
  const del = deps.tg.sent.at(-1).opts.buttons.flat()[3].callback_data; // 🗑
  await router.handleUpdate(cb(del));
  assert.match(deps.tg.sent.at(-1).html, /Удалить встречу\?/);
  const yes = deps.tg.sent.at(-1).opts.buttons[0][0].callback_data;
  await router.handleUpdate(cb(yes));
  assert.match(deps.tg.sent.at(-1).html, /Встреча удалена/);
});

test('кнопки «Нашёл встречу»: Перенести → новое время → перенесена', async () => {
  const deps = makeDeps({
    classifierMap: { 'найди': { intent: 'find', titles: ['тест кнопок'] } },
    gcalOpts: { events: [rawEvent('f3', 'Тест кнопок', '2026-07-24T15:00:00', '2026-07-24T16:00:00')] },
  });
  const router = createRouter(deps);
  await router.refreshCache();
  await router.handleUpdate(msg('найди тест кнопок'));
  const mv = deps.tg.sent.at(-1).opts.buttons.flat()[0].callback_data; // 🔁
  await router.handleUpdate(cb(mv));
  const ask = deps.tg.sent.at(-1);
  assert.match(ask.html, /На какое время перенести/);
  await router.handleUpdate(msg('в 18:00', { reply_to_message: { message_id: ask.message_id, text: ask.html } }));
  assert.match(deps.tg.sent.at(-1).html, /перенесена/);
  const patch = deps.gcal.calls.find((c) => c[0] === 'patch');
  assert.ok(patch[2].start.dateTime.includes('T18:00'));
});

// ── Правка Стаса 23.07: несколько найденных → выбор кнопками с датами → карточка с меню ──
test('find: несколько одинаковых → «С какой работаем?» с датами в кнопках → выбор → карточка с кнопками', async () => {
  const deps = makeDeps({
    classifierMap: { 'найди': { intent: 'find', titles: ['ходьба'] } },
    gcalOpts: { events: [
      rawEvent('w1', 'Вечерняя ходьба', '2026-07-24T21:00:00', '2026-07-24T22:00:00'),
      rawEvent('w2', 'Вечерняя ходьба', '2026-07-25T21:00:00', '2026-07-25T22:00:00'),
    ] },
  });
  const router = createRouter(deps);
  await router.refreshCache();
  ...
  const ask = deps.tg.sent.at(-1);
  assert.match(ask.html, /С какой работаем\?/);
  const btns = ask.opts.buttons.flat();
  assert.equal(btns[0].text, '1 · ПТ, 24 июля');           // день недели и дата в кнопке
  assert.equal(btns[1].text, '2 · СБ, 25 июля');
  await router.handleUpdate(cb(btns[1].callback_data));     // выбираю вторую
  const card = deps.tg.sent.at(-1);
  assert.match(card.html, /Нашёл встречу/);
...
// ── Правка Стаса 24.07: встреча в прошлом — предупреждение с выбором ──
test('прошлое время → «уже в прошлом» с 3 кнопками; «Всё равно» → создаётся', async () => {
  const deps = makeDeps({ classifierMap: {
    'вчерашка': { intent: 'create', title: 'Вчерашка', date: '2026-07-22', time_start: '08:00' },
  } });
  const router = createRouter(deps);
  await router.refreshCache();
  await router.handleUpdate(msg('поставь вчерашка сегодня на 8 утра'));
  const card = deps.tg.sent.at(-1);
  assert.match(card.html, /уже в прошлом/);
  assert.equal(card.opts.buttons[0].length, 3);              // Всё равно / Перенести / Отмена
  await router.handleUpdate(cb(card.opts.buttons[0][0].callback_data));
  assert.match(deps.tg.sent.at(-1).html, /добавлена/);
});

test('прошлое время → «Перенести» → новое время → создаётся', async () => {
  const deps = makeDeps({ classifierMap: {
    'вчерашка': { intent: 'create', title: 'Вчерашка', date: '2026-07-22', time_start: '08:00' },
  } });
  const router = createRouter(deps);
  ...
  await router.handleUpdate(msg('в 18:00', { reply_to_message: { message_id: ask.message_id, text: ask.html } }));
  assert.match(deps.tg.sent.at(-1).html, /добавлена/);
  assert.match(deps.tg.sent.at(-1).html, /18:00/);
});

test('идущая прямо сейчас встреча прошлым НЕ считается', async () => {
  const deps = makeDeps({ classifierMap: {
    'сейчаска': { intent: 'create', title: 'Сейчаска', date: '2026-07-22', time_start: '11:30' },
  } });
  const router = createRouter(deps);
  await router.refreshCache();
  await router.handleUpdate(msg('поставь сейчаска сегодня на 11:30')); // NOW = 12:00, конец в 12:30
  assert.match(deps.tg.sent.at(-1).html, /добавлена/);
});
