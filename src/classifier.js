...
17. В одной фразе НЕСКОЛЬКО РАЗНЫХ создаваемых встреч («каждую СБ Тест 1 в 17:00 и каждое ВС Тест 2 в 18:00») → заполни ВСЕ поля для ПЕРВОЙ встречи, а названия остальных верни списком в "more_titles":["Тест 2"].`;
}
    ...
    if (format === 'openai') {
      const r = await fetchFn(`${baseUrl.replace(/\/$/, '')}/chat/completions`, {
        ...
        body: JSON.stringify({
          ...
          user: 'secretary-bot-v6', // суффикс менять при смене промпта: свежая сессия без старой истории
