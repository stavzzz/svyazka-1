...
// Несколько создаваемых встреч в одной фразе — ставим по одной (приёмка 24.07:
// «Тест 2» терялся молча).
export function rMoreTitles(titles) {
  return head('📝 По одной за раз') +
    `Сначала ставлю первую. ${titles.length > 1 ? 'Остальные' : 'Вторую'} скажи отдельной фразой: <b>${titles.map(esc).join(', ')}</b>.`;
}
